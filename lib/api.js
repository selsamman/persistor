...
module.exports = function (PersistObjectTemplate, baseClassForPersist) {
    ...
    PersistObjectTemplate._injectIntoObject = function (object) {
        ...
        object.setDelete = function (txn, onlyIfChanged, cascade, logger) {
            PersistObjectTemplate.setAsDeleted(this, txn, onlyIfChanged, !cascade, logger);
        };
        ...
        object.fetchByQuery = function(options) {
            options = options || {};
            var logger = options.logger || PersistObjectTemplate.logger;
            logger.debug({component: 'persistor', module: 'api', activity: 'fetchByQuery',
                data: {template: object.__template__.__name__, id: object.__id__}});
        ...
        object.save = function save(options) {
            var txn = options.transaction;
            var cascade = options.cascade;
            var logger = options.logger || PersistObjectTemplate.logger;
            logger.debug({component: 'persistor', module: 'api', activity: 'save',
                data: {template: object.__template__.__name__, id: object.__id__}});

            if (!txn) {
                this.persistSave(null, logger);
            }
            else if (txn && !cascade) {
                object.setDirty(txn, false, false, logger)
            }
            else {
                object.setDirty(txn, false, true, logger);
            }
        };

        object.delete = function (options) {
            var txn = options.transaction;
            var cascade = options.cascade;
            var logger = options.logger || PersistObjectTemplate.logger;
            logger.debug({component: 'persistor', module: 'api', activity: 'delete',
                data: {template: object.__template__.__name__, id: object.__id__}});

            if (!txn) {
                this.persistDelete(null, logger);
            }
            else if (txn && !cascade) {
                object.setDelete(txn, false, false, logger)
            }
            else {
                object.setDelete(txn, false, true, logger);
            }
        };
    };
    ...
    PersistObjectTemplate._injectIntoTemplate = function (template) {
        ...
        template.fetchById = function(id, options) {
            options = options || {};
            (options.logger || PersistObjectTemplate.logger).debug({component: 'persistor', module: 'api', activity: 'fetchById',
                data: {template: template.__name__, id: id}});
            var dbType = PersistObjectTemplate.getDB(PersistObjectTemplate.getDBAlias(template.__collection__)).type;
            return (dbType == PersistObjectTemplate.DB_Mongo ?
        ...
        template.fetch = function(query, options) {
            options = options || {};
            var logger = options.logger || PersistObjectTemplate.logger;
    ...
    PersistObjectTemplate.setAsDeleted = function (obj, txn, onlyIfChanged, noCascade, logger) {
        var topObject;
        // Get array references too
        if (onlyIfChanged && this.MarkChangedArrayReferences) {
            this.MarkChangedArrayReferences();
        ...
        if (txn && obj.__template__.__schema__.cascadeSave && !noCascade) {
            ...
            topObject = PersistObjectTemplate.getTopObject(obj);
            if (!topObject) {
                (logger || this.logger).error({component: 'persistor', module: 'api', activity: 'setAsDeleted'}, 'Warning: setAsDeleted called for ' + obj.__id__ + ' which is an orphan');
            }
            if (topObject && topObject.__template__.__schema__.cascadeSave) {
                PersistObjectTemplate.enumerateDocumentObjects(PersistObjectTemplate.getTopObject(obj), function (obj) {
                    if (!onlyIfChanged || obj.__deleted__) {
    ...
    PersistObjectTemplate.commit = function commit(transaction) {
        var logger = logger || PersistObjectTemplate.logger;
        var persistorTransaction = transaction || this.__defaultTransaction__;

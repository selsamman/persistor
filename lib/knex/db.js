module.exports = function (PersistObjectTemplate) {
    ...
    function synchronizeIndexes(tableName, template) {
        ...
        var diffTable = function(dbschema, schema, tableName) {
            var tabRegEx = new RegExp(tableName, 'i');
            var dbDefKey = Object.keys(dbschema).filter(function(key) {
                var documentOf = dbschema[key].documentOf || '';
                var table = dbschema[key].table || '';
                return (documentOf.match(tabRegEx) || table.match(tabRegEx));
            });
            var dbTableDef = dbschema[dbDefKey];
            var memDefKey = Object.keys(schema).filter(function(key) {
                var documentOf = schema[key].documentOf || '';
                var table = schema[key].table || '';
                return (documentOf.match(tabRegEx) || table.match(tabRegEx));
            });
            var memTableDef = schema[memDefKey];

module.exports = function (PersistObjectTemplate) {
    ...
    PersistObjectTemplate.synchronizeKnexDbFromModel = function (template) {
       ...
       // return discoverColumns(tableName).then(knex.schema.table.bind(this, tableName, columnMapper.bind(this)));
        ...
        function discoverColumns(table) {
            return Q(knex(table).columnInfo().then(function (info) {
                    for (var prop in props) {
                        if (!info.hasOwnProperty(prop)) {
                            newFields[prop] = props[prop];
                        }
                    }
                }));
        }
    ...
    PersistObjectTemplate.createKnexTable = function (template) {
       ...
       // .then(appendKeyConstraints.bind(this))
        //function appendKeyConstraints(table) {
        //    console
        //}

        function createIndexes(table) {
            if (schema.indexes === undefined) return;
            schema.indexes.forEach(function(index){
                    var name = index.name;
                    if (index.def === undefined) return;

                    table[index.def.type](index.def.columns, name);
                   // table[index.def.unique ? "unique" : "index"](index.def.columns, name);
                    //table.primary(['id', 'name'], "srk_test");
                    //table
                    //table.unique(index.def.columns, "kjhkjkh");
                    //table.index(index.def.columns, name )
                }
            )
        }

        function setConstraints(property, tableColumnDefinition)
        {
            tableColumnDefinition.unique();
            //tableColumnDefinition["unique"]("srk_unique");
        }
        ...
        function createColumns(table) {
            ...
            for (var prop in props) {
                ...
                if (defineProperty.type === Array) {
                    ...
                    table.string(prop);
            //    setConstraints(prop, table._statements[table._statements.length - 1]);
            }
            createIndexes(table);
          //  return Q(table);
        }
